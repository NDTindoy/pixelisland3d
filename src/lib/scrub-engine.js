...
function mountScrollWorld(container, config) {
  ...
  if (config.brand) {
    ...
    if (config.brand.logoUrl) {
      ...
      img.alt = config.brand.name || 'Brand Logo';
  ...
  let destroyed = false, rafId = 0, readRafId = 0;
  ...
  function raf() {
    ...
    if (!destroyed) rafId = requestAnimationFrame(raf);
  }
  ...
  const onScroll = () => { if (!ticking) { ticking = true; readRafId = requestAnimationFrame(read); } };
  ...
  // Return an explicit teardown for SPA route changes and React StrictMode.
  // Upstream is a standalone-page engine and intentionally has no lifecycle;
  // without this, every revisit leaves another rAF loop + window listener set.
  function destroy() {
    if (destroyed) return;
    destroyed = true;
    ...
    SEGMENTS.forEach(s => {
      if (s.video) { try { s.video.pause(); } catch {} s.video.removeAttribute('src'); s.video.load(); }
    });
...
// Vendored from the scroll-world skill (references/scrub-engine.js). Unmodified
// except for this export -- keep it that way, since the phone hardening (seek
// coalescing, iOS priming, URL-bar resize suppression) is easy to lose in a rewrite.
// The upstream file also assigned module.exports / window for script-tag use; both
// are dropped here because this is consumed as an ES module by Vite.
